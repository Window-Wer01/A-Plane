(function (root, factory) {
  ...
  const WEB_BGM_SRC = "./assets/bgm-ui-test-004-20260921.wav";
  const WX_BGM_SRC = "assets/bgm-ui-test-004-20260921.wav";
  const WEB_DROP_SFX_SRC = "./assets/sfx-drop-ui-test-004-20260921.wav";
  const WX_DROP_SFX_SRC = "assets/sfx-drop-ui-test-004-20260921.wav";
  const WEB_MERGE_SFX_SRC = "./assets/sfx-merge-ui-test-004-20260921.wav";
  const WX_MERGE_SFX_SRC = "assets/sfx-merge-ui-test-004-20260921.wav";
  ...
  function createWebAudioManager(settings) {
    ...
    return {
      ...
      playDrop() {
        playEffect(WEB_DROP_SFX_SRC, 0.92);
      },
      playMerge(level = 0) {
        playEffect(WEB_MERGE_SFX_SRC, Math.min(1, 0.95 + level * 0.02));
      }
    };
  }
  ...
  function createWxAudioManager(settings, wxApi) {
    ...
    return {
      ...
      playDrop() {
        playEffect(WX_DROP_SFX_SRC, 0.92);
      },
      playMerge(level = 0) {
        playEffect(WX_MERGE_SFX_SRC, Math.min(1, 0.95 + level * 0.02));
      }
    };
  }
  ...
  function initWebShell(doc) {
    ...
    const settings = {
      ...
      volume: clamp(Number(settingsStorage.get("volume", 0.72)) || 0.72, 0, 1)
    };
    ...
    function resizeCanvas() {
      ...
      if (shellRoot) {
        ...
        const stageTop = Math.round(clamp((162 * fitScale) - toolSize * 0.4, 108, 140));
        ...
        const toolBottom = Math.round(clamp((-52 * fitScale) - toolSize * 0.37, -102, -60));
