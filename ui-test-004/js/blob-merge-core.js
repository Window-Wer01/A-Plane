  ...
  class BlobMergeCore {
    ...
    loadMonsterSprites() {
      try {
        ...
        const levelSprites = TYPES.map((_, index) => {
          const level = index + 1;
          const pad = String(level).padStart(2, "0");
          return createSprite(`./assets/ball-lv-${pad}-ui-test-004.png`);
        });
        return { levels: levelSprites };
      } catch {
        return null;
      }
    }
    ...
    getMonsterSprite(typeIndex) {
      ...
      const index = Math.max(0, Math.min(TYPES.length - 1, typeIndex || 0));
      const list = this.monsterSprites.levels;
