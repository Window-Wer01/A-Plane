(() => {
  ...
  const friendRankScreen = document.getElementById("friendRankScreen");
  const petParkScreen = document.getElementById("petParkScreen");
  const gameScreen = document.getElementById("gameScreen");
  const menuStartBtn = document.getElementById("menuStartBtn");
  const menuRankBtn = document.getElementById("menuRankBtn");
  const menuPetBtn = document.getElementById("menuPetBtn");
  ...
  const resultShareBoard = document.getElementById("resultShareBoard");
  const resultShareStatus = document.getElementById("resultShareStatus");
  ...
  const WECHAT_SHARE_STATE_KEY = "blob-merge-prototype-wechat-share-state";
  ...
  const USER_SYNC_ENDPOINT = typeof window !== "undefined" && typeof window.__BLOB_USER_SYNC_URL__ === "string"
    ? window.__BLOB_USER_SYNC_URL__.trim()
    : "";
  ...
  const MAX_SHARE_REVIVES_PER_RUN = 3;
  const REVIVE_STEP_BACK_COUNT = 10;
  const MAX_RUN_SNAPSHOTS = 40;
  const TEST_BGM_MIN_VOLUME = 0.88;
  ...
  MONSTER_IMAGES.yellow.src = "./assets/monster-yellow.png";
  MONSTER_IMAGES.blue.src = "./assets/monster-blue.png";
  MONSTER_IMAGES.green.src = "./assets/monster-green.png";
  ...
  const BGM_TRACK_PATH = "./assets/bgm-paper-boat.mp3?v=standard";
  ...
  function writeBest(value) {
    try {
      ...
      scheduleUserInfoSync("best-score-updated");
    } catch {
      // 忽略本地存储失败
    }
  }
  ...
  function writeKingMinSteps(value) {
    try {
      ...
      scheduleUserInfoSync("best-step-updated");
    } catch {
      // 忽略本地存储失败
    }
  }
  ...
  const ACTIVE_DECAY_WINDOW_MS = 90 * 1000;
  const PET_VIDEO_BUFF_MS = 5 * MINUTE_MS;
  const PET_ONLINE_DECAY_PER_HOUR = {
    energy: 1,
    mood: 1.67,
    clean: 0.83
  };
  const PET_OFFLINE_DECAY_PER_HOUR = {
    energy: 0.5,
    mood: 0.83,
    clean: 0.42
  };
  const PET_BUY_PRICING = {
    energy: { base: 6, limit: 5, extra: 10 },
    mood: { base: 10, limit: 3, extra: 15 },
    clean: { base: 5, limit: 5, extra: 8 }
  };
  const PET_GIFT_POOL = [
    { label: "墨镜", type: "gift" },
  ...
  const TOOL_CONFIG = {
    ...
    rage: {
      label: "发脾气",
      short: "脾气",
      description: "下半区体积前 2 类里任选 1 类并随机放大到 250%"
    },
    split: {
      label: "分裂弹",
      short: "分裂",
  ...
  function getEnergyDecayMultiplier(energyValue) {
    if (energyValue >= 8) return 0.7;
    if (energyValue >= 5) return 1;
    if (energyValue >= 3) return 1.2;
    return 1.4;
  }

  function getMoodGameplayProfile(moodValue) {
    if (moodValue >= 8) {
      return {
        extraWeight: 0.25,
        emoji: "😄",
  ...
  function readDailyState(now = new Date()) {
    const today = getTodayKey(now);
    try {
      const saved = JSON.parse(localStorage.getItem(WECHAT_DAILY_STATE_KEY) || "null");
      if (saved && saved.date === today) {
        return {
          date: today,
          firstRunClaimed: Boolean(saved.firstRunClaimed),
          videoBonusCount: Number(saved.videoBonusCount || 0),
          purchases: {
            energy: Number(saved.purchases?.energy || 0),
            mood: Number(saved.purchases?.mood || 0),
            clean: Number(saved.purchases?.clean || 0)
          }
        };
      }
    } catch {
    ...
    return {
      ...
      videoBonusCount: 0,
      purchases: { energy: 0, mood: 0, clean: 0 }
    };
  }

  function writeDailyState(dailyState) {
    try {
      localStorage.setItem(WECHAT_DAILY_STATE_KEY, JSON.stringify(dailyState));
    } catch {
      // ignore
    }
  }
  ...
  function readShareState() {
    try {
      const saved = JSON.parse(localStorage.getItem(WECHAT_SHARE_STATE_KEY) || "null");
      return {
        lastShareDate: typeof saved?.lastShareDate === "string" ? saved.lastShareDate : "",
        streakDays: Math.max(0, Math.floor(Number(saved?.streakDays || 0))),
        totalShares: Math.max(0, Math.floor(Number(saved?.totalShares || 0))),
        firstDailyClaimedDate: typeof saved?.firstDailyClaimedDate === "string" ? saved.firstDailyClaimedDate : ""
      };
    } catch {
      return {
        lastShareDate: "",
        streakDays: 0,
        totalShares: 0,
        firstDailyClaimedDate: ""
      };
    }
  }

  function writeShareState(shareState) {
    try {
      localStorage.setItem(WECHAT_SHARE_STATE_KEY, JSON.stringify({
        lastShareDate: typeof shareState?.lastShareDate === "string" ? shareState.lastShareDate : "",
        streakDays: Math.max(0, Math.floor(Number(shareState?.streakDays || 0))),
        totalShares: Math.max(0, Math.floor(Number(shareState?.totalShares || 0))),
        firstDailyClaimedDate: typeof shareState?.firstDailyClaimedDate === "string" ? shareState.firstDailyClaimedDate : ""
      }));
    } catch {
      // ignore
    }
  ...
  function getDateDiffDays(prevDateText, nextDateText) {
    if (!prevDateText || !nextDateText) return NaN;
    const prev = new Date(`${prevDateText}T00:00:00`);
    const next = new Date(`${nextDateText}T00:00:00`);
    if (!Number.isFinite(prev.getTime()) || !Number.isFinite(next.getTime())) return NaN;
    return Math.round((next.getTime() - prev.getTime()) / 86400000);
  }

  function getShareRewardBoardData(now = new Date()) {
    const shareState = readShareState();
    const today = getTodayKey(now);
    const dailyFirstAvailable = shareState.firstDailyClaimedDate !== today;
    return {
      today,
      streakDays: shareState.streakDays,
      totalShares: shareState.totalShares,
      dailyFirstAvailable,
      reached3Day: shareState.streakDays >= 3,
      reached7Day: shareState.streakDays >= 7
    };
  }

  function recordShareSuccess(now = new Date()) {
    const today = getTodayKey(now);
    const shareState = readShareState();
    const dayDiff = getDateDiffDays(shareState.lastShareDate, today);
    if (!shareState.lastShareDate) {
      shareState.streakDays = 1;
    } else if (dayDiff === 1) {
      shareState.streakDays += 1;
    } else if (dayDiff > 1) {
      shareState.streakDays = 1;
    }
    shareState.lastShareDate = today;
    shareState.totalShares += 1;
    const isFirstDaily = shareState.firstDailyClaimedDate !== today;
    if (isFirstDaily) {
      shareState.firstDailyClaimedDate = today;
    }
    writeShareState(shareState);
    return {
      shareState,
      isFirstDaily,
      unlocked3Day: shareState.streakDays >= 3,
      unlocked7Day: shareState.streakDays >= 7
    };
  }

  function getCurrentBuyCost(slot) {
    const pricing = PET_BUY_PRICING[slot];
    if (!pricing) return 0;
    const dailyState = readDailyState();
    const count = Number(dailyState.purchases?.[slot] || 0);
    return count < pricing.limit ? pricing.base : pricing.extra;
  }

  function registerDailyPurchase(slot) {
    const dailyState = readDailyState();
    dailyState.purchases[slot] = Number(dailyState.purchases?.[slot] || 0) + 1;
    writeDailyState(dailyState);
  }

  function consumeVideoCoinBonus() {
    const dailyState = readDailyState();
    if (dailyState.videoBonusCount >= 3) return 0;
    dailyState.videoBonusCount += 1;
    writeDailyState(dailyState);
    return 2;
  }
  ...
  function writeGold(value) {
    try {
      ...
      scheduleUserInfoSync("gold-updated");
    } catch {
      // 忽略本地存储失败
    }
  }
  ...
  function createDefaultPetState(now = Date.now()) {
    return {
      energy: { value: 5, lastTick: now, buffUntil: 0 },
      mood: { value: 5, lastTick: now, buffUntil: 0 },
      clean: { value: 5, lastTick: now, buffUntil: 0 },
      gifts: [],
      lastAnimation: {
        emoji: "🐾",
        text: "我一直在等你点电视耶！"
  ...
  function writePetState(petState) {
    try {
      ...
      scheduleUserInfoSync("pet-updated");
    } catch {
      // 忽略本地存储失败
    }
  }
  ...
  function getUserDeviceId() {
    try {
      const existing = localStorage.getItem(WECHAT_USER_DEVICE_ID_KEY);
      if (existing) return existing;
      const created = `wx-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
      localStorage.setItem(WECHAT_USER_DEVICE_ID_KEY, created);
      return created;
    } catch {
      return "wx-anonymous";
    }
  ...
  function readUserInfo() {
    return readJsonStorage(WECHAT_USER_INFO_KEY, null);
  }

  function writeUserInfo(info) {
    writeJsonStorage(WECHAT_USER_INFO_KEY, info);
  }

  function readPendingUserInfo() {
    return readJsonStorage(WECHAT_PENDING_USER_INFO_KEY, null);
  }

  function writePendingUserInfo(info) {
    writeJsonStorage(WECHAT_PENDING_USER_INFO_KEY, info);
  }

  function hasPendingUserInfoSync() {
    return Boolean(readPendingUserInfo());
  }

  function buildUserInfoSnapshot(reason = "state-change") {
    const pet = state.pet || readPetState();
    const progress = readProgress();
    const leaderboard = Array.isArray(state.cachedFriendLeaderboard) && state.cachedFriendLeaderboard.length
      ? state.cachedFriendLeaderboard
      : readWechatRankCache();
    const me = Array.isArray(leaderboard) ? leaderboard.find((row) => row?.isMe) : null;
    const bestSteps = readKingMinSteps();
    return {
      id: getUserDeviceId(),
      nickname: "我",
      ...
      stats: {
        bestScore: readBest(),
        bestSteps: bestSteps > 0 ? bestSteps : null,
        gold: readGold(),
        totalRuns: Number(progress.totalRuns || 0),
        totalScore: Number(progress.totalScore || 0),
        totalMerges: Number(progress.totalMerges || 0),
        rank: Number(me?.rank || 0)
      },
      pet: {
        energy: Number(pet.energy?.value ?? 5),
        mood: Number(pet.mood?.value ?? 5),
        clean: Number(pet.clean?.value ?? 5),
        gifts: Array.isArray(pet.gifts) ? pet.gifts.slice(0, 6) : []
      }
    };
  }

  async function pushUserInfoSnapshot(snapshot) {
    if (!USER_SYNC_ENDPOINT || typeof fetch !== "function") {
      return { mode: "local-only", payload: null };
    }
    const response = await fetch(USER_SYNC_ENDPOINT, {
      method: "POST",
  ...
  function clearUserInfoSyncTimer() {
    if (!userInfoSyncTimer || typeof window === "undefined") return;
    window.clearTimeout(userInfoSyncTimer);
    userInfoSyncTimer = 0;
  }

  async function flushPendingUserInfo(reason = "online-sync") {
    if (!getShellOnline()) return false;
    if (userInfoSyncInFlight) return userInfoSyncInFlight;
    const snapshot = readPendingUserInfo() || buildUserInfoSnapshot(reason);
    userInfoSyncInFlight = pushUserInfoSnapshot(snapshot)
      .then((result) => {
        writePendingUserInfo(null);
        writeUserInfo({
          ...snapshot,
      ...
      .finally(() => {
        userInfoSyncInFlight = null;
      });
    return userInfoSyncInFlight;
  }

  function scheduleUserInfoSync(reason = "state-change", immediate = false) {
    const snapshot = buildUserInfoSnapshot(reason);
    writeUserInfo({
      ...snapshot,
      syncState: getShellOnline() ? "syncing" : "pending"
    });
    if (!getShellOnline()) {
      writePendingUserInfo(snapshot);
      clearUserInfoSyncTimer();
      return;
    }
    writePendingUserInfo(snapshot);
    clearUserInfoSyncTimer();
    const runSync = () => {
      flushPendingUserInfo(reason);
    };
    if (immediate || typeof window === "undefined") {
      runSync();
      return;
    }
    userInfoSyncTimer = window.setTimeout(runSync, 600);
  }

  function splitDecayElapsed(lastTick, now) {
    const elapsed = Math.max(0, now - lastTick);
    const onlineElapsed = Math.min(elapsed, ACTIVE_DECAY_WINDOW_MS);
    const offlineElapsed = Math.max(0, elapsed - onlineElapsed);
    return { onlineElapsed, offlineElapsed };
  }

  function applyDecayWindow(value, elapsedMs, ratePerHour, buffMultiplier) {
    if (elapsedMs <= 0 || ratePerHour <= 0) return value;
    const ratePerMs = ratePerHour / HOUR_MS;
    return clamp(value - elapsedMs * ratePerMs * buffMultiplier, 0, PET_MAX_VALUE);
  }

  function decayAttributeValue(currentValue, lastTick, now, onlineRate, offlineRate, buffUntil = 0, extraMultiplier = 1) {
    const { onlineElapsed, offlineElapsed } = splitDecayElapsed(lastTick, now);
    const onlineStart = now - onlineElapsed;
    let value = currentValue;
    if (offlineElapsed > 0) {
      const offlineBuffMultiplier = buffUntil > onlineStart - offlineElapsed ? 0.5 : 1;
      value = applyDecayWindow(value, offlineElapsed, offlineRate * extraMultiplier, offlineBuffMultiplier);
    }
    if (onlineElapsed > 0) {
      const buffCovered = Math.max(0, Math.min(now, buffUntil) - onlineStart);
      const normalCovered = Math.max(0, onlineElapsed - buffCovered);
      value = applyDecayWindow(value, buffCovered, onlineRate * extraMultiplier, 0.5);
      value = applyDecayWindow(value, normalCovered, onlineRate * extraMultiplier, 1);
    }
    return value;
  }
  ...
  function getPetSnapshot(now = Date.now()) {
    ...
    const energyDecayMultiplier = getEnergyDecayMultiplier(pet.energy.value);
    pet.energy.value = decayAttributeValue(
      pet.energy.value,
      pet.energy.lastTick,
      now,
      PET_ONLINE_DECAY_PER_HOUR.energy,
      PET_OFFLINE_DECAY_PER_HOUR.energy,
      pet.energy.buffUntil
    );
    pet.energy.lastTick = now;

    pet.mood.value = decayAttributeValue(
      pet.mood.value,
      pet.mood.lastTick,
      now,
      PET_ONLINE_DECAY_PER_HOUR.mood,
      PET_OFFLINE_DECAY_PER_HOUR.mood,
      pet.mood.buffUntil,
      energyDecayMultiplier
    );
    pet.mood.lastTick = now;

    pet.clean.value = decayAttributeValue(
      pet.clean.value,
      pet.clean.lastTick,
      now,
      PET_ONLINE_DECAY_PER_HOUR.clean,
      PET_OFFLINE_DECAY_PER_HOUR.clean,
      pet.clean.buffUntil,
      energyDecayMultiplier
    );
  ...
  function awardRunCoins(amount) {
    ...
    state.rewardCoinsEarned += safeAmount;
    addCoins(safeAmount);
  }
  ...
  function readWechatRankCache() {
    try {
      const raw = JSON.parse(localStorage.getItem(WECHAT_RANK_CACHE_KEY) || "[]");
  ...
  function writeWechatRankCache(list) {
    try {
      localStorage.setItem(WECHAT_RANK_CACHE_KEY, JSON.stringify(list));
    } catch {
      // 忽略本地存储失败
    }
  }

  function createMockFriendLeaderboard(forceRefresh = false) {
    const cached = !forceRefresh ? readWechatRankCache() : [];
    if (cached.length) return cached;

    const minSteps = readKingMinSteps();
    const mySteps = minSteps > 0 ? minSteps : 88;
    const seed = [
      ["阿桃", 42], ["小虎", 48], ["团团", 53], ["米露", 56], ["阿嘉", 63],
      ["泡泡", 69], ["小葵", 74], ["北鼻", 81], ["星星", 87], ["阿哲", 95],
      ["夏夏", 101], ["噜噜", 108]
    ];

    const rows = seed.map(([name, steps], index) => ({
      id: `friend-${index + 1}`,
      name,
      steps: steps + (forceRefresh ? Math.floor(Math.random() * 3) : 0),
      isMe: false
    }));
    rows.push({ id: "self", name: "我", steps: mySteps, isMe: true });
    rows.sort((a, b) => a.steps - b.steps);
    rows.forEach((row, index) => {
      row.rank = index + 1;
    });
    writeWechatRankCache(rows);
    return rows;
  }

  function getShellOnline() {
    return typeof navigator === "undefined" ? true : navigator.onLine !== false;
  }

  function showShellScreen(screen) {
    const screenMap = {
      menu: menuScreen,
      game: gameScreen,
      rank: friendRankScreen,
      pet: petParkScreen
    };
  ...
  function renderFriendRankScreen(forceRefresh = false) {
    const online = getShellOnline();
    const rows = createMockFriendLeaderboard(forceRefresh && online);
    state.cachedFriendLeaderboard = rows;
    const topTen = rows.slice(0, 10);
    if (friendLeaderboardList) {
      friendLeaderboardList.innerHTML = topTen.map((row) => `
        <li class="leaderboard-item ${row.isMe ? "is-me" : ""}">
          <span class="leaderboard-rank">#${row.rank}</span>
          <div class="leaderboard-meta">
            <strong>${row.name}${row.isMe ? "（我）" : ""}</strong>
            <small>${row.isMe ? "当前玩家" : "好友"}</small>
          </div>
          <div class="leaderboard-score">
            <span>最少步数</span>
            <strong>${row.steps}</strong>
          </div>
        </li>
      `).join("");
    }
    ...
    if (myRankValue) {
      myRankValue.textContent = me ? `第 ${me.rank} 名` : "第 - 名";
    }
    if (rankSyncState) {
      if (!online) {
        rankSyncState.textContent = hasPendingUserInfoSync() ? "离线使用缓存，用户信息待同步" : "离线使用缓存";
      } else {
        const userInfo = readUserInfo();
        const userSyncText = userInfo?.syncedAt ? "用户信息已更新" : "在线可同步";
        rankSyncState.textContent = forceRefresh ? `已在线刷新，${userSyncText}` : userSyncText;
      }
    }
    return me?.rank || 0;
  }

  function updateShellNotice(text) {
    if (shellNotice) {
  ...
  function syncShellEntryMessage() {
    ...
    const prevRank = Number(localStorage.getItem(WECHAT_LAST_RANK_KEY) || 0);
    const userInfo = readUserInfo();
    const pendingUserInfo = hasPendingUserInfoSync();
    const rank = renderFriendRankScreen(getShellOnline());
    let notice = "你的宠物一直在等你来！";
    if (getShellOnline() && prevRank > 0 && rank > 0 && rank < prevRank) {
      notice = `好友排行上升到第 ${rank} 名啦！`;
    } else if (!getShellOnline() && pendingUserInfo) {
      notice = "当前离线中，核心玩法照常可玩；恢复网络后会自动补发用户信息。";
    } else if (lastVisit > 0 && now - lastVisit > 1000 * 60 * 60 * 6) {
      notice = "你的宠物一直在等你来！";
    } else if (getShellOnline()) {
      notice = userInfo?.syncedAt
        ? "联网中，好友排行榜和用户信息已更新。"
        : "联网中，好友排行榜和用户信息会自动更新。";
    } else {
      notice = "现在没网络也没关系，照样可以继续玩。";
    }

    if (shellNetworkState) {
      shellNetworkState.textContent = getShellOnline() ? "有网络可同步好友榜和用户信息" : "离线也可游玩";
    }
    if (shellWelcomeTip) {
  ...
  function openRewardModal(slot, customCopy = "") {
    ...
    const map = {
      ...
      energy: ["活力频道", "这里未来会接看广告补体力的激励流程。"],
      toy: ["玩具频道", "这里未来会接看广告领取玩具的激励流程。"],
      dress: ["装扮频道", "这里未来会接看广告开装扮宝箱的激励流程。"]
    };
  ...
  function getPetDisplayState(pet) {
    if (pet.mood.value >= 8 && pet.energy.value >= 8) {
      return { emoji: "😄", text: "活力满满，今天特别想跟你一起冲高分。" };
    }
    if (pet.energy.value <= 2 || pet.mood.value <= 2) {
      return { emoji: "🥺", text: "有点撑不住了，点我就能快速补状态。" };
    }
    if (pet.clean.value <= 2) {
      return { emoji: "🫧", text: "屏幕脏脏的，风险线已经压低了。" };
    }
    if (pet.energy.value <= 4) {
      return { emoji: "😴", text: "有点累了，发射节奏会被拖慢。" };
    }
  ...
  function renderPetSystem() {
    ...
    if (petEnergyStatus) {
      ...
      const decayMultiplier = getEnergyDecayMultiplier(pet.energy.value);
      petEnergyStatus.textContent = `${energyRule.text} 快乐/清洁衰减倍率 ×${decayMultiplier.toFixed(1)}，当前补点 ${getCurrentBuyCost("energy")} 金币。`;
    }
    ...
    if (petCleanStatus) {
      ...
      petCleanStatus.textContent = `当前${cleanProfile.text}，风险线高度 ${Math.round(cleanProfile.ratio * 100)}%，${cleanProfile.effect}，补点 ${getCurrentBuyCost("clean")} 金币。`;
    }
    ...
    if (gamePetEmoji) gamePetEmoji.textContent = displayState.emoji;
  ...
  function applyPetAction(slot, action) {
    ...
    if (action === "watch") {
      ...
      if (slot === "energy") {
        ...
        pet.energy.lastTick = now;
        pet.energy.buffUntil = now + PET_VIDEO_BUFF_MS;
        setPetAnimation("🍪", "宠物正在吃饼干，体力恢复了 2 点。");
        openRewardModal("energy", "本次看视频获得体力 +2，且接下来 5 分钟体力衰减减半。");
      } else if (slot === "mood") {
        pet.mood.value = clamp(pet.mood.value + 1, 0, PET_MAX_VALUE);
        pet.mood.lastTick = now;
        pet.mood.buffUntil = now + PET_VIDEO_BUFF_MS;
        setPetAnimation("⚽", "宠物正在玩球摇摆，开心度提升了 1 点。");
        openRewardModal("energy", "本次看视频获得开心度 +1，且接下来 5 分钟快乐衰减减半。");
      } else if (slot === "clean") {
        pet.clean.value = clamp(pet.clean.value + 2, 0, PET_MAX_VALUE);
        pet.clean.lastTick = now;
        pet.clean.buffUntil = now + PET_VIDEO_BUFF_MS;
        setPetAnimation("🛁", "宠物正在洗澡刷牙，清洁度恢复了 2 点。");
        openRewardModal("dress", "本次看视频获得清洁度 +2，且接下来 5 分钟清洁衰减减半。");
      }
      bonusCoins = consumeVideoCoinBonus();
      if (bonusCoins > 0) {
        addCoins(bonusCoins);
      }
      writePetState(pet);
    ...
    if (action === "buy") {
      const cost = getCurrentBuyCost(slot);
      if (!cost) return;
      if (!spendCoins(cost)) {
        openRewardModal("gift", "金币不够，先去游戏里拿奖励，或者看电视累积资源。");
        return;
      }
      registerDailyPurchase(slot);
      if (slot === "energy") {
        pet.energy.value = clamp(pet.energy.value + 1, 0, PET_MAX_VALUE);
        pet.energy.lastTick = now;
        setPetAnimation("🥠", `你花了 ${cost} 金币补了 1 点体力。`);
      } else if (slot === "mood") {
        pet.mood.value = clamp(pet.mood.value + 1, 0, PET_MAX_VALUE);
        pet.mood.lastTick = now;
        setPetAnimation("💃", `你花了 ${cost} 金币补了 1 点开心度。`);
      } else if (slot === "clean") {
        pet.clean.value = clamp(pet.clean.value + 1, 0, PET_MAX_VALUE);
        pet.clean.lastTick = now;
        setPetAnimation("🪥", `你花了 ${cost} 金币补了 1 点清洁度。`);
      }
      writePetState(pet);
      renderPetSystem();
      return;
  ...
  function weightedRandomLevel() {
    ...
    const adjustedPool = openingPool.map((item) => {
      ...
      if (moodProfile.extraWeight > 0) {
        if (item.level >= 3) {
          weight *= 1 + moodProfile.extraWeight;
        } else if (item.level <= 1) {
          weight *= 1 - moodProfile.extraWeight * 0.35;
        }
      }
      return { ...item, weight };
    });
  ...
  function writeLeaderboard(entries) {
    try {
      ...
      scheduleUserInfoSync("leaderboard-updated");
    } catch {
      // ignore
    }
  }
  ...
  function writeProgress(progress) {
    try {
      ...
      scheduleUserInfoSync("progress-updated");
    } catch {
      // ignore
    }
  }
  ...
  function ensureBgmAudio() {
    if (bgmAudio) {
      bgmAudio.volume = Math.max(TEST_BGM_MIN_VOLUME, Math.max(0, Math.min(1, settings.volume)));
      return bgmAudio;
    }
    ...
    audio.volume = Math.max(TEST_BGM_MIN_VOLUME, Math.max(0, Math.min(1, settings.volume)));
    audio.muted = false;
    audio.defaultMuted = false;
    audio.addEventListener("canplaythrough", () => {
      if (state.started && !state.paused && !state.gameOver && settings.audioEnabled) {
        syncBgm();
      }
    });
    audio.addEventListener("pause", () => {
      if (state.started && !state.paused && !state.gameOver && settings.audioEnabled && document.visibilityState !== "hidden") {
        window.setTimeout(() => syncBgm(), 120);
      }
    });
    audio.load();
    bgmAudio = audio;
  ...
  function forceStartBgmPlayback(options = {}) {
    ...
    settings.volume = Math.max(TEST_BGM_MIN_VOLUME, Math.max(0, Math.min(1, settings.volume)));
    saveSettings();
    syncSettingsUI();
  ...
  function serializeBlob(blob) {
    return {
      ...
      vx: Number(blob.vx || 0),
      vy: Number(blob.vy || 0),
      r: Number(blob.r || LEVELS[blob.level || 0].radius),
      age: Number(blob.age || 0),
      mood: blob.mood,
      blinkOffset: Number(blob.blinkOffset || 0),
      blinkTimer: Number(blob.blinkTimer || 0),
      blinkCooldown: Number(blob.blinkCooldown || 0),
      pulse: Number(blob.pulse || 0),
      nodPhase: Number(blob.nodPhase || 0),
      settledTime: Number(blob.settledTime || 0),
      expression: blob.expression || "idle",
      expressionTimer: Number(blob.expressionTimer || 0),
      smileSeed: Number(blob.smileSeed || 0),
      creatureVariant: blob.creatureVariant || null,
      baseR: Number(blob.baseR || LEVELS[blob.level || 0].radius),
      specialType: blob.specialType || null,
      specialArmed: Boolean(blob.specialArmed)
    };
  }
  ...
  function deserializeBlob(data) {
    return {
      ...
      vx: Number(data.vx || 0),
      vy: Number(data.vy || 0),
      r: Number(data.r || LEVELS[data.level || 0].radius),
      age: Number(data.age || 0),
      mood: data.mood || FACE_MOODS[0],
      blinkOffset: Number(data.blinkOffset || 0),
      blinkTimer: Number(data.blinkTimer || 0),
      blinkCooldown: Number(data.blinkCooldown || 1.6),
      pulse: Number(data.pulse || 0),
      nodPhase: Number(data.nodPhase || 0),
      settledTime: Number(data.settledTime || 0),
      expression: data.expression || "idle",
      expressionTimer: Number(data.expressionTimer || 0),
      smileSeed: Number(data.smileSeed || 0),
      creatureVariant: data.creatureVariant || getCreatureVariantForLevel(Number(data.level || 0)),
      baseR: Number(data.baseR || LEVELS[data.level || 0].radius),
      specialType: data.specialType || null,
      specialArmed: Boolean(data.specialArmed)
    };
  }

  function captureRunSnapshot() {
    if (!state.started || state.paused || state.gameOver) return;
    state.runSnapshots.push({
      elapsedMs: state.runStartTime ? Math.max(0, performance.now() - state.runStartTime) : 0,
      blobs: state.blobs.map((blob) => serializeBlob(blob)),
      pointerX: state.pointerX,
      nextLevel: state.nextLevel,
      score: state.score,
      dropCount: state.dropCount,
      mergeCount: state.mergeCount,
      centerDropCount: state.centerDropCount,
      edgeDropCount: state.edgeDropCount,
      firstMergeAt: state.firstMergeAt,
      firstWarningAt: state.firstWarningAt,
      recoveryCount: state.recoveryCount,
      peakWarningLevel: state.peakWarningLevel,
      maxBlobsOnBoard: state.maxBlobsOnBoard,
      triggeredDangerCount: state.triggeredDangerCount,
      warningLevel: state.warningLevel,
      overlineTime: state.overlineTime,
      dropCooldown: state.dropCooldown,
      idleSinceDrop: state.idleSinceDrop,
      maxLevelReached: state.maxLevelReached,
      lastDroppedBlobId: state.lastDroppedBlobId,
      idSeed: state.idSeed,
      capsuleTimer: state.capsuleTimer,
      rageTimer: state.rageTimer,
      rageRecoverTimer: state.rageRecoverTimer,
      rageBlobId: state.rageBlobId,
      splitBombArmed: state.splitBombArmed,
      toolInventory: { ...getToolInventory() },
      toolRunUsage: { ...getToolRunUsage() }
    });
    if (state.runSnapshots.length > MAX_RUN_SNAPSHOTS) {
      state.runSnapshots.splice(0, state.runSnapshots.length - MAX_RUN_SNAPSHOTS);
    }
  }

  function restoreRunSnapshot(snapshot) {
    if (!snapshot) return false;
    state.blobs = (snapshot.blobs || []).map((blob) => deserializeBlob(blob));
    state.popups = [];
    state.bursts = [];
    state.sparks = [];
    state.pointerX = Number(snapshot.pointerX || PIT.x + PIT.width / 2);
    state.pointerActive = false;
    state.nextLevel = Number(snapshot.nextLevel || 0);
    state.score = Number(snapshot.score || 0);
    state.started = true;
    state.paused = false;
    state.gameOver = false;
    state.dropCount = Number(snapshot.dropCount || 0);
    state.mergeCount = Number(snapshot.mergeCount || 0);
    state.centerDropCount = Number(snapshot.centerDropCount || 0);
    state.edgeDropCount = Number(snapshot.edgeDropCount || 0);
    state.firstMergeAt = Number(snapshot.firstMergeAt || 0);
    state.firstWarningAt = Number(snapshot.firstWarningAt || 0);
    state.recoveryCount = Number(snapshot.recoveryCount || 0);
    state.peakWarningLevel = Number(snapshot.peakWarningLevel || 0);
    state.maxBlobsOnBoard = Number(snapshot.maxBlobsOnBoard || state.blobs.length);
    state.triggeredDangerCount = Number(snapshot.triggeredDangerCount || 0);
    state.warningLevel = Number(snapshot.warningLevel || 0);
    state.overlineTime = Number(snapshot.overlineTime || 0);
    state.dropCooldown = Number(snapshot.dropCooldown || 0);
    state.idleSinceDrop = Number(snapshot.idleSinceDrop || 0);
    state.maxLevelReached = Number(snapshot.maxLevelReached || 0);
    state.lastDroppedBlobId = snapshot.lastDroppedBlobId ?? null;
    state.idSeed = Number(snapshot.idSeed || 1);
    state.capsuleTimer = Number(snapshot.capsuleTimer || 0);
    state.rageTimer = Number(snapshot.rageTimer || 0);
    state.rageRecoverTimer = Number(snapshot.rageRecoverTimer || 0);
    state.rageBlobId = snapshot.rageBlobId ?? null;
    state.splitBombArmed = Boolean(snapshot.splitBombArmed);
    state.toolInventory = createDefaultToolInventory();
    Object.assign(state.toolInventory, snapshot.toolInventory || {});
    writeToolInventory(state.toolInventory);
    state.toolRunUsage = createDefaultToolRunUsage();
    Object.assign(state.toolRunUsage, snapshot.toolRunUsage || {});
    ...
    state.resultOutcome = "fail";
    state.runStartTime = performance.now() - Math.max(0, Number(snapshot.elapsedMs || 0));
  ...
  function revokeLastSettlementReward() {
    const rollbackCoins = Math.max(0, Math.floor(Number(state.rewardCoinsEarned || 0)));
    if (rollbackCoins > 0) {
      const previousCoins = state.coins;
      state.coins = Math.max(0, state.coins - rollbackCoins);
      writeGold(state.coins);
      updateCoinDisplays({ animate: false, previousValue: previousCoins });
      renderPetSystem();
    }
    state.rewardCoinsEarned = 0;
    state.settlementRewardBase = 0;
  ...
  function renderResultShareBoard() {
    if (!resultShareBoard || !resultShareStatus) return;
    const board = getShareRewardBoardData();
    const reviveRemain = Math.max(0, MAX_SHARE_REVIVES_PER_RUN - state.shareReviveUsed);
    resultShareBoard.innerHTML = `
      <span class="result-share-chip ${board.dailyFirstAvailable ? "is-ready" : "is-done"}">每日首次 ${board.dailyFirstAvailable ? "待领取" : "已记录"}</span>
      <span class="result-share-chip ${board.reached3Day ? "is-done" : ""}">连续 3 天 ${board.reached3Day ? "已达成" : `进度 ${Math.min(board.streakDays, 3)}/3`}</span>
      <span class="result-share-chip ${board.reached7Day ? "is-done" : ""}">连续 7 天 ${board.reached7Day ? "已达成" : `进度 ${Math.min(board.streakDays, 7)}/7`}</span>
    `;
    if (state.successRun) {
      resultShareStatus.textContent = `分享通关卡片后会记录今日分享与连续天数，奖励细则待补充。当前连续 ${board.streakDays} 天，总分享 ${board.totalShares} 次。`;
    } else {
      resultShareStatus.textContent = `本局可分享复活 ${reviveRemain} 次，成功后会回退到结束前 ${REVIVE_STEP_BACK_COUNT} 步的局面。当前连续分享 ${board.streakDays} 天。`;
    }
  }

  async function performShareAction(mode = "success") {
    const title = mode === "success" ? "我要当大王通关啦" : "我要当大王差一点就过关";
    const text = mode === "success"
      ? `我刚通关，用时 ${formatRunTimer(Math.max(1, getRunSeconds()))}，准备分享给朋友圈。`
      : `我这局准备分享复活，回到结束前 ${REVIVE_STEP_BACK_COUNT} 步继续挑战。`;
    const url = typeof window !== "undefined" ? window.location.href : "";
    if (typeof navigator !== "undefined" && typeof navigator.share === "function") {
      try {
        await navigator.share({ title, text, url });
        return { shared: true, mode: "system", ...recordShareSuccess() };
      } catch (error) {
    ...
    if (typeof navigator !== "undefined" && navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
      try {
        ...
        return { shared: true, mode: "clipboard", ...recordShareSuccess() };
      } catch {
        // ignore
      }
    }
    return { shared: true, mode: "test", ...recordShareSuccess() };
  }

  async function handleResultPrimaryAction() {
    const shareMode = state.successRun ? "success" : "revive";
    if (!state.successRun && state.shareReviveUsed >= MAX_SHARE_REVIVES_PER_RUN) {
      setStatus("本局分享复活次数已用满 3 次");
      renderResultShareBoard();
      return;
    }
    const shareResult = await performShareAction(shareMode);
    if (!shareResult.shared) {
      if (shareResult.cancelled) {
        setStatus("你先取消了分享，准备好再试一次。");
      }
      return;
    }
    if (state.successRun) {
      renderResultShareBoard();
      if (resultReviewTip) {
        resultReviewTip.textContent = `分享成功，已记录今日分享。当前连续 ${shareResult.shareState.streakDays} 天，后续每日首次、连续 3 天、连续 7 天奖励细则待补充。`;
      }
      setStatus("通关卡片已分享成功，今日分享记录已更新。");
      return;
    }
    if (!state.runSnapshots.length) {
      setStatus("没有可回退的局面，先再开一局试试。");
      return;
    }
    const targetIndex = Math.max(0, state.runSnapshots.length - REVIVE_STEP_BACK_COUNT);
    const snapshot = state.runSnapshots[targetIndex];
    revokeLastSettlementReward();
    state.shareReviveUsed = Math.min(MAX_SHARE_REVIVES_PER_RUN, state.shareReviveUsed + 1);
    state.runSnapshots = state.runSnapshots.slice(0, targetIndex + 1);
    if (!restoreRunSnapshot(snapshot)) {
      setStatus("分享成功了，但复活回退失败，请再来一局。");
      return;
    }
    if (resultReviewTip) {
      resultReviewTip.textContent = `分享复活成功，已回到结束前 ${Math.min(REVIVE_STEP_BACK_COUNT, targetIndex + 1)} 步的局面。`;
    }
    renderResultShareBoard();
    activateBgmFromGesture({ keepPlaying: true });
    setStatus(`分享复活成功，已回退到结束前 ${Math.min(REVIVE_STEP_BACK_COUNT, targetIndex + 1)} 步。`);
  }
  ...
  function updateDangerUI() {
    ...
    const cleanProfile = state.pet ? getCleanRiskProfile(state.pet.clean.value) : getCleanRiskProfile(5);
    ...
    if (state.gameOver) {
      ...
      text = state.warningLevel > 0.58 ? "接近红线，优先清理上层" : `风险线处于 ${Math.round(cleanProfile.ratio * 100)}% 高度`;
    } else {
      ratio = clamp(state.warningLevel * 0.45, 0, 0.24);
      text = `当前安全，清洁状态为${cleanProfile.text}`;
    }
  ...
  function useRageTool() {
    ...
    target.r = (target.baseR || LEVELS[target.level].radius) * 2.5;
    target.vx *= 0.85;
    target.vy *= 0.85;
  ...
  function handleGameOver(outcome = "fail") {
    ...
    const durationSeconds = Math.max(1, getRunSeconds());
    const baseRewardCoins = computeRunCoinReward(outcome, state.dropCount);
    const dailyState = readDailyState();
    const firstRunBonus = outcome === "success" && !dailyState.firstRunClaimed ? 3 : 0;
    if (firstRunBonus > 0) {
      dailyState.firstRunClaimed = true;
      writeDailyState(dailyState);
    }
    ...
    if (resultDoubleRewardBtn) {
      resultDoubleRewardBtn.textContent = outcome === "success" ? "分享通关卡片" : `分享复活（${Math.max(0, MAX_SHARE_REVIVES_PER_RUN - state.shareReviveUsed)}/${MAX_SHARE_REVIVES_PER_RUN}）`;
    }
    if (resultReviewTip) {
      const dailyText = firstRunBonus > 0 ? "，含首局奖励 +3" : "";
      resultReviewTip.textContent = outcome === "success"
        ? `通关成功，本局用时 ${formatRunTimer(durationSeconds)}，结算奖励 ${settlementCoins}${dailyText}。分享成功卡片后会记录每日首次分享与连续分享天数，奖励细则待补充。`
        : `本局失败，用时 ${formatRunTimer(durationSeconds)}，当前奖励 ${settlementCoins} 已入账。点击“分享复活”后可回退到结束前 ${REVIVE_STEP_BACK_COUNT} 步，本局最多 ${MAX_SHARE_REVIVES_PER_RUN} 次。`;
    }
    renderResultShareBoard();
    updateHud();
    updateDangerUI();
  ...
  function updatePhysics(dt) {
    ...
    if (state.started && state.time % 1 < dt) {
      syncBgm();
    }
    const runSeconds = getRunSeconds();
    if (runSeconds >= 60) {
    ...
    if (!state.gameOver && state.started) {
      if (hasStackCandidate) {
        const dynamicWarningY = getWarningLineY();
        const dynamicDangerY = getDangerLineY();
        const warning = clamp((dynamicWarningY - stackTop) / (dynamicWarningY - PIT.y), 0, 1);
  ...
  function drawBackground() {
    ...
    const bloom = ctx.createRadialGradient(WORLD.width * 0.5, PIT.y - 8, 16, WORLD.width * 0.5, PIT.y - 8, 120);
    bloom.addColorStop(0, "rgba(99,102,241,0.22)");
    bloom.addColorStop(0.5, "rgba(99,102,241,0.08)");
    ...
    if (guideCreatureImage.complete && guideCreatureImage.naturalWidth) {
      ctx.save();
      ctx.globalAlpha = 0.16;
      ctx.drawImage(guideCreatureImage, 26, FLOOR_Y - 162, 76, 92);
      ctx.globalAlpha = 0.2;
      ctx.drawImage(guideCreatureImage, 96, FLOOR_Y - 255, 170, 206);
      ctx.globalAlpha = 0.17;
      ctx.drawImage(guideCreatureImage, 254, FLOOR_Y - 198, 122, 148);
      ctx.globalAlpha = 0.1;
      ctx.drawImage(guideCreatureImage, 302, FLOOR_Y - 124, 54, 66);
      ctx.restore();
    }
    ...
    ctx.setLineDash([8, 8]);
    ctx.beginPath();
    ctx.moveTo(PIT.x + 10, dangerY);
    ctx.lineTo(PIT.x + PIT.width - 10, dangerY);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.restore();
    ...
    if (!isMobileMode) {
      ...
      ctx.fillText("危险线", PIT.x + 12, dangerY - 8);
    }

    if (state.warningLevel > 0.02) {
      const warningHeight = Math.max(0, warningY - PIT.y);
  ...
  function drawRabbitGuide(x, y) {
    ...
    const throwLift = state.pointerActive ? 2 : 0;
    const bodyY = y + 18 + throwLift;
    const drawW = 62;
    const drawH = 70;
    ...
    ctx.rotate(headTilt);
    ctx.fillStyle = "rgba(69,53,173,0.24)";
    ctx.beginPath();
    ctx.ellipse(0, -9, 42, 24, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.drawImage(guideCreatureImage, -drawW / 2, -drawH * 0.88, drawW, drawH);
  ...
  function drawCreatureBlob(blob, preview = false) {
    if (!blob.creatureVariant) {
      return false;
    }
    const image = MONSTER_IMAGES[blob.creatureVariant];
  ...
  function drawBlob(blob) {
    ...
    if (state.gameOver && blob.y - blob.r < getDangerLineY()) {
  ...
  function drawGameOverOverlay() {
    ...
    ctx.fillText(`用时 ${formatRunTimer(getRunSeconds())} · 步数 ${state.dropCount}`, WORLD.width / 2, 350);
  ...
  menuExitBtn?.addEventListener("click", () => {
    if (window.history.length > 1) {
      ...
      window.alert("小游戏环境里这里会返回微信，目前先作为退出占位。");
    }
  });
  ...
  scheduleUserInfoSync("page-load", true);
  syncShellEntryMessage();
  if (typeof window !== "undefined") {
    window.addEventListener("online", () => {
      scheduleUserInfoSync("network-restored", true);
      renderFriendRankScreen(true);
      syncShellEntryMessage();
    });
    window.addEventListener("offline", () => {
      scheduleUserInfoSync("network-lost", true);
      syncShellEntryMessage();
    });
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "hidden") {
        scheduleUserInfoSync("page-hidden", true);
      }
    });
    window.addEventListener("beforeunload", () => {
      scheduleUserInfoSync("page-unload", true);
    });
  }
