(function (root, factory) {
  ...
  class BlobMergeCore {
    ...
    render() {
      ...
      background.addColorStop(0, "#16899a");
      background.addColorStop(0.48, "#0f7789");
      background.addColorStop(1, "#07566f");
      ctx.fillStyle = background;
      ctx.fillRect(0, 0, this.viewport.width, this.viewport.height);

      const tile = Math.max(18, Math.round(this.viewport.width / 18));
      ctx.save();
      ctx.globalAlpha = 0.12;
      for (let y = 0; y < this.viewport.height; y += tile) {
        for (let x = 0; x < this.viewport.width; x += tile) {
          ctx.fillStyle = ((x / tile + y / tile) & 1) ? "#b7fff0" : "#053c59";
    ...
    drawScene(ctx) {
      if (this.platform.domUi) {
        ctx.fillStyle = "rgba(5, 70, 87, 0.14)";
    ...
    drawPit(ctx) {
      ...
      pitGradient.addColorStop(0, "#168b99");
      pitGradient.addColorStop(0.55, "#117889");
      pitGradient.addColorStop(1, "#0a6077");
      ctx.fillStyle = pitGradient;
      this.roundRect(ctx, PIT.x, PIT.y, PIT.width, PIT.height, 28, true, false);

      ctx.save();
      this.roundRect(ctx, PIT.x, PIT.y, PIT.width, PIT.height, 28, false, false);
      ctx.clip();
      const tile = 48;
      ctx.globalAlpha = 0.1;
      for (let y = PIT.y; y < FLOOR_Y; y += tile) {
        for (let x = PIT.x; x < PIT.x + PIT.width; x += tile) {
          ctx.fillStyle = ((x / tile + y / tile) & 1) ? "#c3fff1" : "#063f59";
          ctx.fillRect(x, y, tile, tile);
        }
      }
      const dots = [
        [54, 210, 16, "#ff83bd"], [152, 238, 11, "#ffe85a"], [654, 226, 13, "#63d8df"],
        [92, 372, 9, "#8ee55d"], [610, 356, 15, "#ff83bd"], [182, 494, 12, "#63d8df"],
        [558, 518, 10, "#ffe85a"], [78, 650, 13, "#ff8a62"], [664, 682, 15, "#8ee55d"],
        [142, 816, 10, "#ffe85a"], [590, 842, 14, "#63d8df"], [350, 904, 13, "#ff83bd"]
      ];
      ...
      const stars = [[238, 200], [685, 302], [112, 560], [520, 710], [270, 860]];
      ...
      ctx.strokeStyle = "rgba(211, 255, 249, 0.62)";
      ctx.lineWidth = 3;
